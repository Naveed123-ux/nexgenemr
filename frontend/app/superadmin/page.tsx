...
import { fetchAllHospitals, fetchDashboardStats, toggleHospitalStatus } from "@/store/slices/superAdminslice";
...
export default function SuperAdminDashboard() {
  ...
  const { Allhospitals, loading, error, total, page, pageSize, totalPages, stats } =
  ...
  useEffect(() => {
    if (!hasFetched.current) {
      const fetchData = async () => {
        try {
          await Promise.all([
            dispatch(fetchAllHospitals({ page: 1 })).unwrap(),
            dispatch(fetchDashboardStats()).unwrap(),
          ]);
          hasFetched.current = true;
        } catch (error) {
          console.error("Fetch error:", error);
          toast.error("Failed to fetch dashboard data", { id: "dashboard-error" });
        }
      };
      fetchData();
    }
  ...
  const handleToggleStatus = async (id: number) => {
    try {
      await dispatch(toggleHospitalStatus(id)).unwrap();
  ...
  const columns: ColumnDef<Allhospital>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="py-3 px-2 md:px-4 text-sm table-cell">
        ...
        return (
          <div className="flex gap-1 md:gap-2 py-3 px-2 md:px-4">
            <Button
              size="sm"
              variant="outline"
              className={`h-8 px-2 flex items-center gap-1 text-xs ${is_active ? "text-red-500 hover:text-red-700 border-red-200" : "text-green-500 hover:text-green-700 border-green-200"}`}
              onClick={() => handleToggleStatus(id)}
            >
              {is_active ? (
                <><XCircle className="w-3 h-3 md:w-4 md:h-4" /> Deactivate</>
              ) : (
                <><CheckCircle className="w-3 h-3 md:w-4 md:h-4" /> Activate</>
              )}
            </Button>
          </div>
        );
          ...
          <Card className="bg-white shadow-sm">
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                <div className="text-[#388fe5] text-2xl font-bold">
            ...
            <CardContent>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2 mt-4">
                <div
                  ...
                  style={{ width: `${(stats.oldHospitals / stats.totalHospitals) * 100 || 0}%` }}
                ></div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2 mt-4">
                <div
                  ...
                  style={{ width: `${(stats.newHospitalsToday / stats.totalHospitals) * 100 || 0}%` }}
                ></div>
              </div>
            </CardContent>
          </Card>
