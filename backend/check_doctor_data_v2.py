...
with engine.connect() as conn:
    print("--- ASSOCIATED DATA CHECK (CORRECTED) ---")
    for uid in u_ids:
        print(f"USER ID: {uid}")
        ...
        if doc:
            ...
            print(f"  - Appointments (primary): {appts}")
            
            # Sessions
            sessions = conn.execute(text(f"SELECT count(*) FROM sessions WHERE doctor_id = {pid}")).scalar()
